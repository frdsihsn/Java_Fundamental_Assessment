...
public class Main
{
    ...
    private static void gradeStudent( StudentService studentService, CourseService courseService, Scanner scanner )
    {
        ...
        if(!student.isAttendingCourse(courseId)){
            System.out.println("Student not in " + courseId);
            return;
        }
        ...
        if(grade >=50){
            studentService.registerApprovedCourse(student, course);
            courseService.unEnrollStudent(courseId, student);
            System.out.println(studentId + " completed " + courseId + "!");
        } else{
            System.out.println(studentId + " has to retake " + courseId);
        }
    }
